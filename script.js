...
$(document).ready(function () {
    ...
    function searchInput(cityName) {
        //city search URL  
        // once search function works...add " + cityName + " where Orlando is down below
        const queryURL = "https://api.openweathermap.org/data/2.5/weather?q=Orlando&appid=596ff6a8b9059e68a3a5b2f8821c0145";
        ...
        $.ajax({
            ...
            let cityFahrenheit = $("<p>").text("Temperature: " + tempUp + "°F").addClass("lead");
            ...
            for (let i = 1; i < 6; i++) {
                $.ajax({
                    ...
                    let fiveDayTemp = $("<p>").text("Temp: " + fiveDayResponse.daily[i].temp.max + "°F");
                    fiveDayTemp.attr("id", "#fiveDayTemp[i]");

                    let fiveHumidity = $("<p>").attr("id", "humDay").text("Humidity: " + JSON.stringify(fiveDayResponse.daily[i].humidity) + "%");
